...
## 9. Review (Da Dok, 2026-09-22) — **REJECTED**

3-Seiten-Prüfung Plan↔Code↔Docs über I1 `b04f0e3`, I2 `26538b0`, AGENTS-DEV-Fakten
`d301d5e`. Intake-`lintDocsAndTests` (128 Docs, 153 Tests) lief; Build beider
Plugin-Projekte grün (nur Pre-Existing-Warnings).

### Rework-Item (einziger Blocker)
1. **`UNBELEGT_ERLEDIGT UC-JD-1`** (`docs/java-debugger-tool.md:38`) — **Regression aus
   diesem Zyklus**. `JavaDebugToolTest.java:27` ist jetzt die Komposit-Zeile
   `// UC-JD-1 — list_breakpoints (R-JD-11) is the deliberate session exception, tested in
   DebugListBreakpointsTest.` — `TestParser.extractCommentIds`
   (`docslinter/TestParser.java:100-117`) verlangt, dass der Kommentar-Körper **vollständig**
   der ID-Pattern entspricht; jeder Anhang bricht die Evidenz-Verknüpfung still. UC-JD-1 war
   in den beiden Vor-Zyklen **nicht** flagged (da noch reine `// UC-JD-1`) → eindeutig
   Zyklus-Ursache.
   - **Fix (1 Zeile):** Satz auf eigene Kommentarzeile unterhalb der reinen `// UC-JD-1`.
   - **Wurzelursache = Plan-interner Widerspruch:** I2-Test-Posten schrieb „`// UC-JD-1`-
     Kommentar (:26) um einen Satz ergänzen", §5 verlangt „reine ID-Zeile". Da Thinka:
     Plan-Text korrigieren, damit der Widerspruch nicht wiederkommt.

### Verifiziert ✅ (kein Befund)
- **I1 (R-JD-12/UC-JD-14):** `displayHitCount` (`DebugJson.java:266-268`) einzige
  Clamp-Stelle, genutzt `:252` + List-Nodes `:303/:313`; `breakpointResponse` sonst
  unverändert. Beide UC-JD-14-Tests (`DebugJsonUnitTest.java:465-492`) in beide Richtungen
  mit Output-Assertion (rendered Wert + „raw −1 muss nicht leaken") — Test-Honesty ok.
- **I2 (R-JD-11/UC-JD-13):** `listBreakpoints` `@Tool` ohne Parameter, ohne Session-Guard
  (14 Guards intakt, `:333-338` + `noSession`-Stil `onProblem`/return `:334-336`);
  public static Seam `:348` mit Javadoc (R-JD-1-Ausnahme benannt, Präzedenz
  `primaryTypeName`); Marker-Scan `:348-362` exakt wie Plan (Projekt-DEPTH_INFINITE,
  Root-DEPTH_ZERO); `breakpointListResponse` `:275-288` = exakte Plan-Shape inkl.
  Scope-Disclosure auch bei leerer Liste; `NO_PROJECT`-Konstante `:60`; Wiring
  `SharedToolsComponent` Field `:39`/`addTool` `:70`/Accessor `:116` +
  `PeonAiService.setProject` Push `:261` (dieselbe Quelle wie workspace-Tools);
  Klassen-Javadoc `:43-50` mit Ausnahmeklausel; @Tool-Description deckt alle
  SOLL-Punkte (Phantom-BPs, workspace-wide Exception-BPs, `0 = every hit`,
  `remove_breakpoint`, no-session, no-project-Fehler).
- **Tests:** `DebugListBreakpointsTest` — 5 Tests, alle mit reinen `// UC-JD-13`,
  self-verifying (echte Marker, Attribute beidseitig assertet), Cleanup in `finally`;
  `JavaDebugToolTest.listBreakpointsDoesNotFailWithoutSession` `:64-79` (assertet
  „no project selected" **und** kein „no active debug session"); 14er-
  `noSessionFailsHonest`-Matrix unverändert (list_breakpoints korrekt aussortiert).
- **Beide IST-Abweichungen von Da Mek unabhängig verifiziert — Evidenzqualität gut,
  kein Befund:** (1) `JDTDebugConstants` existiert auf jdt.debug 3.26.100 (2026-07)
  nicht (readTypeSource: Type not found); Marker-Typ-IDs + `typeName`/`hitCount`-
  Attribute in der Plattform-`plugin.xml` bestätigt
  (`.bundle_pool/plugins/org.eclipse.jdt.debug_3.26.100.v20260714-1432/plugin.xml`,
  `javaLineBreakpointMarker` :129, `javaExceptionBreakpointMarker` :171).
  `org.eclipse.jdt.debug.core.condition` steht in **keiner** plugin.xml
  (binary-only, 20 Jahre stabiler JDT-Key; IMarker-Javadoc kennt keine
  Undeclared-Attribute-Persistenz-Limitierung; Verhalten wird von den grünen
  self-verifying Tests bewiesen) → akzeptabel. (2) `IBreakpoint.ENABLED` =
  `org.eclipse.debug.core.enabled` via readTypeSource auf
  `org.eclipse.debug.core.model.IBreakpoint` bestätigt; `IMarker.ATTR_ENABLED` existiert
  nicht. Beide Fakten dokumentiert in `AGENTS-DEV.md:150-158` (`d301d5e`), konsistent
  mit Code-Kommentaren.
- **Docs↔Code (SOLL == IST):** R-JD-11/UC-JD-13 + R-JD-12/UC-JD-14
  (`java-debugger-tool.md:178-202`) vollständig umgesetzt; UC-Status korrekt **❌**
  gelassen (Flip = Jon).
- **Scope-Creep:** `remove_breakpoint` intakt (`:307-330`), keine neuen @P-Parameter,
  keine neuen Dependencies; Suite-Arithmetik 264 + 2 (I1) + 6 (I2) = 272 ✓.
  Suite-Claim selbst nicht neu gefahren (PDE-Trust-Dialog blockt kompletten Re-Run);
  Evidenz = Build-grün + Quellen-Review + Arithmetik.

### Plan coverage gap (Item 4)
Kein SOLL-Lückengerade. **Plan-interner Widerspruch** (§5 „reine ID-Zeile" vs I2-Test-
Posten) ist die Ursache des Rework-Items — kein Coverage-Fehlbefund gegen Da Mek.

### Skill/instruction gap (Item 9)
`AGENTS-DEV.md` (Test-Honesty-Sektion) dokumentiert nicht, dass UC-ID-Test-Kommentare
**reine ID-Zeilen** sein müssen (`TestParser.extractCommentIds` = Full-Match des
Kommentar-Körpers; Anhang = still verlorene Evidenz). Nötige Änderung: diese
Einschränkung in `AGENTS-DEV.md` aufnehmen (Empfehlung an Jon, kein Rework-Item).

### Risk-Linie
Wahrscheinlichster späterer Bruch: die Linter-Regression ist Symptom der
undokumentierten Pure-ID-Einschränkung — jeder künftige Plan, der „Kurzkommentar an die
`// UC-x`-Zeile hängen" anweist, bricht UC-Evidenz still (UNBELEGT_ERLEDIGT blockt
dann Flips). ÄNDERUNG, die das Risiko am meisten senkt: die 1-Zeilen-Kommentar-Umlegung
**plus** die Pure-ID-Regel in `AGENTS-DEV.md` verankern.

**Resolution (2026-09-22):** Rework-Item behoben in `66c1050` (Pure-ID-Zeile + AGENTS-DEV-Konvention); Lint clean; Jon: Flips + Docs-Commit `cda6335` — Zyklus abgeschlossen.
