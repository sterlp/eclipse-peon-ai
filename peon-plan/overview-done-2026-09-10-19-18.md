...
## 10. Review — Da Dok (2026-09-10, Pre-Mortem, frischer Kontext)

**Verdict: REJECTED** — Code ist Plan- und Doc-konform (Evidence unten); blockierend ist genau
ein fehlender inc-2-Deliverable plus falsche DONE-Begründung in §8.

### Rework items (blocking)

1. **inc-2-Doc-Update fehlt** (Plan §4-Tabelle + §5 inc-2 Schritt 2): `docs/chat-job-lifecycle.md:6-8`
   steht weiterhin auf `❌ specified (2026-09-10) … Build pending` (SOLL: `✅ gebaut — User-Smoke
   ausstehend`); `docs/index.md` (Bullet „Chat Job Lifecycle") ebenfalls weiterhin `❌ specified …
   Build pending`. Da Meks §8-Begründung „keine Doc-Änderungen nötig — PO-Docs waren in inc-1
   bereits synchron" ist am IST widerlegt (beide Status-Stellen ❌). Zusätzlich STOP-AND-ASK-Verstoß:
   geplanter Schritt still übergangen statt askDev. Fix: beide Status-Stellen auf
   `✅ gebaut — User-Smoke ausstehend`, eigener Mini-Commit inkl. docs/**.

### Non-blocking (CONCERNS — mit Rework 1 ziehbar oder separat)

- **ERROR-Log-Format-Drift:** Code AIChatView.java:602 `unbalanced in-flight turn counter: <n> —
  cleaning up` folgt dem inc-1-Snippet; Plan §2.8 spezifiziert `(<value>)` — Plan-intern
  widersprüchlich. Beim nächsten Doc-Touch §2.8 ans Code-Format angleichen (kosmetisch).
- **Plan §7-Tabelle:** R-ST2-Zeilen verweisen auf „Smoke 4", die Smoke-Liste nummeriert
  R-ST2/Abort als 5 (4 = Compress+Send). Numerierungs-Drift.
- **Doc R-ST1-Snippet** (chat-job-lifecycle.md, SOLL-Codeblock) zeigt noch die alte Skip-Form
  `turn finally skipped: agent=… (run still in flight)` statt der normativen R-ST3-Form —
  Ticket-Relikt `run=7` ist entfernt ✅; Snippet nur illustrativ. Optional harmonisieren.

### Plan↔Code — verifiziert (Evidence)

- Counter-Feld AIChatView.java:91-93; **genau 2** Increments: submitAiJob:573 + doCompressContext:480
  (beide vor `lockWhileWorking(true)`:575/:482, vor `.schedule()`:589/:502); **genau 2**
  `handleDoneChatResponse`-Call-Sites: :494 (compress, mit onCommitUi-Replay) + :586 (submit, null)
  → 1:1-Paarung grep-verifiziert.
- Commit-Entscheidung vollständig im `runInUiThread`-Runnable (:600-619): decrement :601,
  ERROR `<0` :602, fail-open `<=0` :603, onCommitUi vor Reset :605→:606,
  `monitorRef.set(new NullProgressMonitor())` im UI-Runnable :606, commit-INFO nur bei `==0`
  :612-614, Skip-INFO `remaining+1->remaining` :617 („eigen" = Wert vor decrement, §2.8 ✓).
- Agent-Capture beim Submit: :572 (`final var agent`) / :478 (`active`) → agentName-Param :592 ✓ R-ST3.
- Compress-Guard: alter asyncExec-Block im finally entfernt, Replay als onCommitUi :494-499,
  läuft nur im Commit-Pfad ✓ §2.6; Skip → kein Replay.
- R-ST2 kein Code-Change ✅: Stop-Handler `() -> getIProgressMonitor().setCanceled(true)`
  AIChatView.java:144 (unverändert); frischer Null-Monitor pro Commit :606 → Cancel-Flag nicht klebrig.
- Core unberührt ✅: AbstractAgent.call:177 (CAS-Reject → queue + return null), Queue-Drain
  186-199 im selben `call()`, handleAbortAndDrain:213-220 — Phantom-Job-Annahme code-verifiziert.
- Submit-/Skip-/Commit-Log-Formate exakt §2.8 (außer ERROR-Drift oben).

### Docs↔Code — BDDs

R-ST1 alle 5 BDDs, R-ST2 beide BDDs, R-ST3 Submit/Skip/Commit im Code abgebildet (immer-an, nicht
debug-gated; nur das pre-existing „Chatreponse"-Log bleibt gated = IST). Rest-Fenster
Submit→Body-Start dokumentiert-unfixt wie vorgesehen. Verifikationsmodell (manuell + Code-Review,
R-UI1/R-MCP3-Präzedenz) konsistent; Smoke 1-7 decken die BDDs (abgesehen von der Numerierungs-Drift
oben). Backlog (Send-during-Compress-Memory-Race, submitTurnJob-Dedup, Log-Retention) korrekt
out of scope.

### Docs↔Plan — Coverage

Vollständig: Counter-Invarianten (1:1, never-below-0 fail-open+ERROR, Cancel kein Sonderpfad),
Phantom-Job-Loch, Compress-Guard inkl. vorgeschaltetem asyncExec, keine Core-Klasse, core
unberührt, Doc-/index-Status (geplant, dann fehlend → Rework 1). **Kein Plan-coverage-gap.**

### Mutation-Check (Pflicht, nebenläufige Logik)

- **Die eine Stelle:** Commit-Guard `if (remaining <= 0)` + Skip-Else im UI-Runnable,
  AIChatView.java:601-618. **Mutation:** Guard entfernen (stale finally committet immer).
  **Nachweis ohne SWT-Harness (deterministisch):** zwei überlappende Turns (Send + sofort Compact
  bzw. Doppel-Send im Schedule-Fenster) → zweites finally dekrementiert auf -1 → ERROR-Zeile
  `unbalanced in-flight turn counter: -1` in der Error Log View + doppelte
  `turn done … (in-flight 1->0)` + UI unlockt während 🟢 (Smoke-2/3-Repro). Die `<0`-ERROR-Zeile
  ist der eingebaute Mutations-Detektor — genau dafür lohnt der Beweis. Empfehlung an PO: als
  Smoke-3-Variante mit erwarteten Log-Zeilen dokumentieren. (Gegenmutation „decrement zurück auf
  den Job-Thread" = TOCTOU, nur per Timing-Race nachweisbar — nicht der Kandidat.)

### Skill-Evolution-Outcome

Kein Regel-Loch: STOP-AND-ASK (Plan-Header + memory #26) deckt genau diesen Fall — der Verstoß ist
ein Regelverstoß (still übergangener Plan-Schritt, falsche Begründung in §8), kein fehlender Skill.
Keine Skill-Datei-Änderung nötig; Erinnerung an Da Mek: DONE-Claims brauchen IST-Evidenz pro
Plan-Zeile.

**Most likely reason this breaks later:** DONE-Statusberichte werden nicht Zeile-für-Zeile gegen
den IST verifiziert (§8 behauptete docs-synchron, IST: ❌/Build pending) — so verschwinden geplante
Schritte still. **Change that most reduces that risk:** Ein Inkrement gilt erst als DONE, wenn
jede betroffene Plan-Datei mit IST-Evidenz (Status-String/Zeile) bestätigt ist; sonst STOP-AND-ASK.