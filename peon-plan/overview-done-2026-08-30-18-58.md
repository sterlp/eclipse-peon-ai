...
## 11. Review-Protokoll (Da Thinka, 2026-08-30) — **OK, Freigabe für PO-Abnahme**

Geprüft: Commits 6e34cb2 / bd41c95 / 24a7910 (Branch `new-config`) gegen diesen Plan, genau einmal.

1. **Inkrement 1–3 vollständig:** `MockLlmServer` +`/v1/messages` (Raw-Capture, immer SSE),
   +`/api/chat` (Raw-Capture, NDJSON `done:true`-Finale), +`rootUrl()` — **additive-only**, alle
   bestehenden public Members unverändert (API-Break-Ausschluss für `ModelListFetchTest` etc.).
   `PerAgentConnectionE2ETest` mit S1/S2 3×2 + S3 (3 Stubs, `agentStub.reset()`-Stale-Nachweis).
   `ToolLoopRequestConnectionTest` + `AiServicePerAgentModelTest` weg, Workspace-Grep: keine toten
   Referenzen; `StreamMock` weiterhin von 5 Testklassen genutzt (nicht tot).
2. **BDD §5 1:1:** alle vier Testnamen exakt vorhanden (`inheritsBaseUrl_landsAtBaseStub_withAgentModelAndThink`,
   `agentOwnUrl_landsAtAgentStub_withConfigOnTheWire`, `configEdit_routesToNewUrl_noStaleConnection`,
   `shouldStreamAnthropicSseAndCaptureBody`, `shouldStreamOllamaNdjsonAndCaptureBody`); S2 deckt die
   extraBody-Semantik je Provider (OpenAI User-wins `cache_control` + Reserved-Strip via
   `model=="claude-mock"`, Anthropic baked `foo`, Ollama `has("foo")==false`); S3 geht den echten
   UI-Pfad `withModelConfig → updateConfig → planAgentConfig` (`AgentModelConfig(url, apiKey, model, …)`
   -Reihenfolge verifiziert).
3. **Constraints:** kein Main-Touch außer `MockLlmServer` (`ConfiguredChatModel`, `EffectiveConnection`,
   die 3 Provider neu gelesen = byte-identisch zu Pre-Build-Reads); keine neuen Deps; JUnit5/AssertJ,
   GIVEN/WHEN/THEN, class-level `@Timeout(30)`, frische Instanzen `@BeforeEach`/`@AfterEach` (+
   try/finally für stub3).
4. **Unangetastet:** `AiServicePerAgentThinkTest`, `ModelConnectionCacheTest`, `ModelListFetchTest`
   vorhanden; Suite grün.
5. **Teststand:** eigenständiger Volllauf nach `eclipseBuildProject(llmpeon-core)` (clean, nur
   vorbestehende Warnings): **512 Tests, 0 Failures** — deckt den §6-Delta exakt (507 +2 −4 +6 +1).
   Hinweis (non-blocking): Dev-Report „491 grün" weicht 21 vom realen Eclipse-Invocations-Count ab
   (Zähl-Artefakt; Code-Zustand unabhängig verifiziert grün).

Keine Lücken → kein Delta-Plan. `planImplemented` verbleibt beim Dev-Agent nach PO-Review.
