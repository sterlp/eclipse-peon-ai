...
- **R2 ❌ specified** Read-only-Agenten (Da Thinka, Da Dok) reporten den Outcome **im Reply**;
  ...
  `skillRead`, fixt er im selben Turn.

### Create / Update / Delete — der Evolve

- **R3 ❌ specified** Jon wendet nach jedem Review-Zyklus den skill-evolution-Loop an — genau ein
...
- **R5 ❌ specified** Änderungen an Skills (Create/Update/Delete inkl. Begründung in einer Zeile)
...
- **R6 ❌ specified** Kein Built-in-Prompt (po.txt, review-agent.txt) wird geändert — der Loop
  ...
  Built-in-Prompts = späterer eigener User-Schritt.
- **R7 ❌ specified** Retro-Relation („memory vs. Skill"): cross-projekt-dauerhafte
...
- R2–R7 = AGENTS-PO.md / AGENTS-DEV.md Text (projekt-lokal) — kein Java außer R1.
