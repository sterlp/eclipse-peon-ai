...
* [Advanced Configuration](advanced-configuration.md) - the two-page preference split and per-agent model resolution via `ChatRequest.modelName()`. **2026-08-21:** SOLL agent-spezifischer Config-Umbau (alles pro Agent, inkl. JSON extra body) — [caching.md](caching.md). **2026-08-28 (❌ specified):** per-agent Model-Config (URL/Key/Modell/Think/JSON-Body) + Connection-Cache nach Identität — [ADR-0034](adr/0034-connection-cache-by-identity.md); Modell-Dropdown + Think aus Chat-UI raus → Config-Seite (+ Refresh-Button). Core/UI-Trennung (Config-Domäne in core), kompletter Config-Rebuild bei Update, Custom Agents via AGENT.md-yml.
...
* [Model Loading](model-loading.md) - model list lifecycle: lazy fetch, persistence across agent switches, fallback on failure. **2026-08-28 (❌ specified):** Dropdown wandert in die Config-Seite; Liste einmalig pro Verbindungs-Identität gecacht (ADR-0034).
