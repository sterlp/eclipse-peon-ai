...
* [Agent-API-Retry (Mini)](agents-retry.md) - **Gebaut & grün (nicht committed).** Eigene Klasse `streaming.ApiRetry` am Choke-Point `ToolLoopRequest.call` für **alle Agenten & Kontexte**: verdiente Geduld (`credit` +1/Erfolg, max 10) + `retryCount` (Reset/Erfolg) → linearer Backoff `min(retryCount·10s, maxWait)`, Sleep in `min(1s, restWait)`-Häppchen, Cancel nie retryt (Klassifikation), Root im Wrapper geloggt, englische `onProblem`-Meldung. Löst [retry-mvp.md](retry-mvp.md) ab.
* [API-Retry MVP](retry-mvp.md) - **Überholt** durch [agents-retry.md](agents-retry.md) (alter fixes-N-Entwurf, kann gelöscht werden).
* [Peon-PO (Jon)](po-agent-jon.md) - docs-owning business-owner agent that designs features and orchestrates its own Peon-Plan/Peon-Dev via talkPlan/planWithPlanAgent/askDev/buildWithAgent (+ searchAgent) with planComplete/planImplemented completion signals.
