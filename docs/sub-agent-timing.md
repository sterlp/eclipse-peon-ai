...
| --- | --- | --- | --- |
| `JonDelegateTool` (`talkPlan` / `planWithPlanAgent` / `askDev` / `buildWithAgent`) | Jon's Peon-Plan / Peon-Dev slave | **Da Thinka** / **Da Mek** | `Da Thinka done. (3s)` / `Da Mek done. (12s)` |
...
reads keep the plain role names (`Peon-Plan slave returned no result`, `Search agent failed …`). So the
