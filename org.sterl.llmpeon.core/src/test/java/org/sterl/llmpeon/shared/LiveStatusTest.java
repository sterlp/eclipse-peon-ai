...
class LiveStatusTest {
    ...
    void start_shows_started_time_waiting_and_zero_rate() {
        ...
        // WHEN
        LiveStatus status = LiveStatus.of(start, 0, startedAt.toEpochMilli());
    ...
    void under_a_minute_shows_seconds_only() {
        ...
        // WHEN
        LiveStatus status = LiveStatus.of(answer, 0, 45_000);
        // THEN
    ...
    void under_an_hour_shows_minutes_and_seconds() {
        ...
        OnPartialAiResponse answer = chunk(Type.ANSWER, Instant.EPOCH, 0);
        // WHEN
        LiveStatus status = LiveStatus.of(answer, 0, 195_000);
        // THEN
    ...
    void over_an_hour_shows_hours_minutes_seconds() {
        ...
        OnPartialAiResponse answer = chunk(Type.ANSWER, Instant.EPOCH, 0);
        // WHEN
        LiveStatus status = LiveStatus.of(answer, 0, 3_845_000);
        // THEN
    ...
    void phase_suffix_matches_chunk_type() {
        ...
        long now = 195_000;
        // WHEN / THEN
        assertThat(LiveStatus.of(chunk(Type.THINK, startedAt, 0), 0, now).state()).contains("thinking...");
        assertThat(LiveStatus.of(chunk(Type.ANSWER, startedAt, 0), 0, now).state()).contains("responding...");
        assertThat(LiveStatus.of(chunk(Type.TOOL, startedAt, 0), 0, now).state()).contains("using tools...");
    }
    ...
    void end_shows_done_and_no_rate() {
        ...
        // WHEN
        LiveStatus status = LiveStatus.of(end, 0, 100_000);
    ...
    void rate_is_tokens_over_token_phase() {
        ...
        long now = 2_000_000;
        OnPartialAiResponse answer = chunk(Type.ANSWER, Instant.EPOCH, now - 2_000);
        // WHEN
        LiveStatus status = LiveStatus.of(answer, 1000, now);
        // THEN 1000 / 2 s = 500
    ...
    void rate_burst_is_tokens_over_token_phase() {
        ...
        long now = 2_000_000;
        OnPartialAiResponse answer = chunk(Type.ANSWER, Instant.EPOCH, now - 200);
        // WHEN
        LiveStatus status = LiveStatus.of(answer, 500, now);
    ...
    void rate_zero_while_token_phase_not_started() {
        ...
        OnPartialAiResponse answer = chunk(Type.ANSWER, Instant.EPOCH, 0);
        // WHEN
        LiveStatus status = LiveStatus.of(answer, 100, 5_000);
        // THEN
        assertThat(status.tokPerSec()).isZero();
    }
    ...
    void elapsed_from_turn_rate_from_token_phase() {
        ...
        OnPartialAiResponse answer = chunk(Type.ANSWER, startedAt, now - 2_000);
        // WHEN
        LiveStatus status = LiveStatus.of(answer, 1000, now);
